...
import { updateEmail } from "@/app/(protected)/services/emailTemplate/api";
...
const EditModal = <T extends Record<string, any>>({
  ...
  const handleClose = () => {
    onClose();
    router.push("/template/template-section/email", { scroll: false });
  };
  ...
  const handleUpdate = async () => {
    ...
    try {
      const result = await updateEmail(id, editedData.name,editedData.isActive);
      if (result.success) {
