...
import { deleteWatti } from "@/app/(protected)/services/wattiTemplate/api";
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const handleDelete = async (id: string) => {
        try {
          const result = await deleteWatti(id);
          if (result.success) {
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  ...
                  onClick={() =>
                    router.push(`/template/template-section/watti?id=${row.original.id}`)
                  }
                >
