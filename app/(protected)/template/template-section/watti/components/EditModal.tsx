...
import { updateWatti } from "@/app/(protected)/services/wattiTemplate/api";
...
const EditModal = <T extends Record<string, any>>({
  ...
  const handleClose = () => {
    onClose();
    router.push("/template/template-section/watti", { scroll: false });
  };
  ...
  const handleUpdate = async () => {
    ...
    try {
      const result = await updateWatti(id, editedData.name,editedData.isActive);
      if (result.success) {
